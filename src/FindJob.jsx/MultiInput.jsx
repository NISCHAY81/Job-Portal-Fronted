    ...
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput
          ...
          leftSection={
            <div className="text-bright-sun-400 p-1 bg-mine-shaft-900 rounded-full mr-3">
              <props.icon />
            </div>
          }
        >
          <Pill.Group>
            {value.length > 0 && (
              <>
                ...
                {values[0]}
                {/* if more than 1 selected, show counter */}
            ...
            <Combobox.EventsTarget>
              <PillsInput.Field
                ...
                value={search}
                placeholder={value.length === 0 ? `${props.title}` : ''}  // ✅ FIXED placeholder
                onChange={(event) => {
      ...
      <Combobox.Dropdown>
        <Combobox.Options>
          ...
          {search.trim().length > 0 && options.length === 0 && exactOptionMatch && (
