  ...
  return (
    <div className='bg-mine-shaft-900 hover:bg-mine-shaft-850 transition-colors duration-200 p-5 w-full max-w-xs flex flex-col gap-4 rounded-2xl shadow-md hover:shadow-lg border border-mine-shaft-800 hover:border-bright-sun-400'>
      ...
      <div className='flex justify-between items-start'>
        <div className='flex gap-3 items-center'>
          <div className='p-2 bg-mine-shaft-800 rounded-lg'>
            <Avatar className='h-7 w-7 object-contain' src={`/${image}.png`} alt={name} />
          </div>
          <div>
            <div className='font-semibold text-bright-sun-400 text-sm'>{role}</div>
            <div className='text-lg font-semibold'>{name}</div>
            <div className='text-xs text-mine-shaft-400'>{company}</div>
          </div>
        </div>
      ...
      <div className='flex flex-wrap gap-2 mt-10 text-xs'>
        ...
        <div className='py-1 px-2 bg-mine-shaft-800 text-bright-sun-400 rounded-lg'>{location}</div>
      </div>
      ...
      {invited ? (
        ...
        <div className='flex justify-between items-center text-sm'>
          <div className='font-semibold text-bright-sun-400'>{expectedCtc}</div>
          <div className='flex gap-1 items-center text-mine-shaft-400'>
            <IconMapPin className='h-4 w-4' stroke={1.5} /> {postedDaysAgo || Math.floor(Math.random() * 10) + 1} days ago
          </div>
        </div>
      )}
      ...
      <Modal opened={opened} onClose={close} title="Schedule Interview" centered>
        <div className='flex flex-col gap-4'>
          ...
          <TimeInput
            ...
            value={time}
            onChange={setTime}
            onFocus={(e) => e.target.showPicker?.()} // ensures time picker popup
          />
